...
import {
  TUTORIAL_FILTER_OPTIONS,
  type TutorialFilterOption,
} from "@/lib/project-images";
import { cn } from "@/lib/utils";

type TutorialFilterChipsProps = {
  value: TutorialFilterOption;
  onChange: (value: TutorialFilterOption) => void;
  className?: string;
};

export function TutorialFilterChips({
  value,
  onChange,
  className,
}: TutorialFilterChipsProps) {
    ...
    <div ref={rootRef} className={cn("relative", className)}>
      ...
      {open && (
        <ul
          ...
          className="absolute right-0 top-[calc(100%+8px)] z-20 min-w-[240px] overflow-hidden rounded-[24px] border border-foreground/10 bg-card p-2 shadow-[0_20px_60px_rgba(0,0,0,0.25)] dark:border-white/10"
        >
